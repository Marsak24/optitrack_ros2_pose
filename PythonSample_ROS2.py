...
rclpy.init()
ros_node = rclpy.create_node("optitrack_publisher")
pose_pub = ros_node.create_publisher(PoseStamped, "/optitrack/pose", 10)
...
def receive_rigid_body_frame(new_id, position, rotation):
    ...
    msg.pose.orientation.x = float(rotation[0])
    msg.pose.orientation.y = float(rotation[1])
    msg.pose.orientation.z = float(rotation[2])
    msg.pose.orientation.w = float(rotation[3])
